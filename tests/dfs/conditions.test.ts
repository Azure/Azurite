...
import ConditionalHeadersAdapter from "../../src/dfs/conditions/ConditionalHeadersAdapter";
import ConditionResourceAdapter from "../../src/dfs/conditions/ConditionResourceAdapter";
import ReadConditionalHeadersValidator from "../../src/dfs/conditions/ReadConditionalHeadersValidator";
import WriteConditionalHeadersValidator from "../../src/dfs/conditions/WriteConditionalHeadersValidator";
import StorageErrorFactory from "../../src/dfs/errors/StorageErrorFactory";
...
describe("ReadConditionalHeadersValidator for exist resource", () => {
  it("Should return 412 preconditoin failed for failed if-match results @loki @sql", () => {
    ...
    const expectedError = StorageErrorFactory.getConditionNotMet(context);
    try {
      validator.validate(
        context,
      ...
      assert.deepStrictEqual(error.errorCode, expectedError.errorCode);
      assert.deepStrictEqual(error.errorMessage, expectedError.errorMessage);
      return;
    }
  ...
  it("Should return 304 Not Modified for failed if-none-match results @loki @sql", () => {
    ...
    const expectedError = StorageErrorFactory.getNotModified(context);
    try {
      validator.validate(
        context,
        new ConditionalHeadersAdapter(context, modifiedAccessConditions),
...
describe("ReadConditionalHeadersValidator for unexist resource", () => {
  ...
  it("Should return 400 getUnsatisfiableCondition for if none-match value * @loki @sql", () => {
    ...
    const expectedError =
      StorageErrorFactory.getUnsatisfiableCondition(context);
...
describe("WriteConditionalHeadersValidator for unexist resource", () => {
  it("Should throw 400 Bad Request for invalid combinations conditional headers @loki @sql", () => {
    ...
    const expectedError =
      StorageErrorFactory.getMultipleConditionHeadersNotSupported(context);

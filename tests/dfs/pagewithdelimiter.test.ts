...
import { BlobPrefixModel } from "../../src/dfs/persistence/IDataLakeMetadataStore";
import PageWithDelimiter from "../../src/dfs/persistence/PageWithDelimiter";
...
describe("PageWithDelimiter", () => {
  ...
  const createReader = (
    ...
    return (o: number) => {
      return Promise.resolve(items.slice(o, o + maxResults));
    };
  };
  ...
  describe("with no delimiter", () => {
    ...
    it("fills exact count with no continuation @Loki @sql", async () => {
      ...
      const [items, prefixes, marker] = await page.fill(
        createReader(blobs, blobs.length),
        namer
      );
    ...
    it("fills smaller than max page with no continuation @Loki @sql", async () => {
      ...
      const [items, prefixes, marker] = await page.fill(
        createReader(blobs, blobs.length + 1),
        namer
      );
      checkResult(items, prefixes, marker, blobs.length, 0, "");
    });
  ...
  describe("with '/' delimiter", () => {
    describe("and 1 item page size", () => {
      ...
      it("returns 1 of 2 items with proper continuation @Loki @sql", async () => {
        const blobs = ["a", "b"];
        const page = new PageWithDelimiter<string>(1, "/");
        let [items, prefixes, marker] = await page.fill(
          createReader(blobs, 1),
          namer
        );
        checkResult(items, prefixes, marker, 1, 0, "a");
        ...
        page.reset();
        [items, prefixes, marker] = await page.fill(
          createReader(blobs.slice(1), 1),
          namer
        );
        checkResult(items, prefixes, marker, 1, 0, "");
      });

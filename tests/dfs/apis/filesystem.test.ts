...
describe("DataLakeFileSystemClient", () => {
  ...
  it("setMetadata with tracing @loki @sql", async () => {
    const metadata = {
      key0: "val0",
      keya: "vala",
      keyb: "valb"
  ...
  it("create with encryption scope @loki @sql", async function (this: Context) {
  ...
  it("create with encryption scope - preventEncryptionScopeOverride : false @loki @sql", async function (this: Context) {
    let encryptionScopeName;
    try {
      encryptionScopeName = getEncryptionScope();
    } catch {
  ...
  it("createIfNotExists @loki @sql", async () => {
    const cClient = serviceClient.getFileSystemClient(
      getUniqueName(fileSystemName)
    );
    const metadata = { key: "value" };
  ...
  it("deleteIfExists @loki @sql", async () => {
    const cClient = serviceClient.getFileSystemClient(
      getUniqueName(fileSystemName)
    );
  ...
  it("listPaths with default parameters @loki @sql", async () => {
    const recordedNow = new Date();

    const fileClients = [];
    for (let i = 0; i < 3; i++) {
      const fileClient = fileSystemClient.getFileClient(
        getUniqueName(`file${i}`)
      );
    ...
    assert.equal(
      result.pathItems![0].lastModified?.getUTCFullYear(),
      recordedNow.getUTCFullYear()
    );
    assert.equal(
      result.pathItems![0].lastModified?.getUTCMonth(),
      recordedNow.getUTCMonth()
    );
    assert.equal(
      result.pathItems![0].lastModified?.getUTCDate(),
      recordedNow.getUTCDate()
    );
    assert.equal(
      result.pathItems![0].lastModified?.getUTCHours(),
      recordedNow.getUTCHours()
    );
  ...
  it("listPaths - Encryption Scope @loki @sql", async function (this: Context) {
    let encryptionScopeName;
    try {
      encryptionScopeName = getEncryptionScope();
    } catch {
    ...
    const result = (await cClient.listPaths().byPage().next())
      .value as FileSystemListPathsResponse;
  ...
  it("listPaths - PagedAsyncIterableIterator with Encryption Scope @loki @sql", async function (this: Context) {
    let encryptionScopeName;
    try {
      encryptionScopeName = getEncryptionScope();
    } catch {
  ...
  it("listPaths - ExpiryTime, NeverExpire @loki @sql", async function (this: Context) {
    const fileClient = fileSystemClient.getFileClient(getUniqueName(`file`));
    await fileClient.create();
  ...
  it.skip("listPaths - ExpiryTime, Absolute [listPaths don't return expiresOn in the spec and adding it make some other sdks fail] @loki @sql", async function (this: Context) {
    const now = new Date();
    const delta = 30 * 1000;
    ...
    assert.equal(
      ...
      recordedExpiresOn.getTime()
    );
    await fileClient.delete();
  });

  it.skip("listPaths - ExpiryTime, RelativeToNow [listPaths don't return expiresOn in the spec and adding it make some other sdks fail] @loki @sql", async () => {
    const delta = 30 * 1000;
    const fileClient = fileSystemClient.getFileClient(getUniqueName(`file`));
  ...
  it.skip("listPaths - ExpiryTime, RelativeToCreation [listPaths don't return expiresOn in the spec and adding it make some other sdks fail] @loki @sql", async () => {
  ...
  it("listPaths with default parameters - null path shouldn't throw error @loki @sql", async () => {
    const fileClients = [];
    for (let i = 0; i < 3; i++) {
      const fileClient = fileSystemClient.getFileClient(
        getUniqueName(`file${i}`)
    ...
    const result = (
      await fileSystemClient.listPaths({ path: "" }).byPage().next()
    ).value;
    assert.deepStrictEqual(result.continuation, undefined);
    assert.deepStrictEqual(result.pathItems!.length, fileClients.length);
    assert.ok(fileClients[0].url.indexOf(result.pathItems![0].name));
  ...
  it("listPaths with all parameters configured @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator for listPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator(generator .next() syntax) for listPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator(byPage()) for listPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator(byPage() - continuationToken) for listPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("verify fileSystemName passed to the client @loki @sql", async () => {
  ...
  it("exists returns true on an existing file system @loki @sql", async () => {
    const result = await fileSystemClient.exists();
    assert.ok(
      result,
      "exists() should return true for an existing file system"
    );
  });

  it("exists returns false on non-existing file system @loki @sql", async () => {
    const newFileSystemClient = serviceClient.getFileSystemClient(
      getUniqueName("newfilesystem")
    );
    const result = await newFileSystemClient.exists();
    assert.ok(
      result === false,
      "exists() should returns false on non-existing file system"
    );
  });
});
...
describe("DataLakeFileSystemClient with soft delete", () => {
  ...
  it("listDeletedPaths with default parameters @loki @sql", async () => {
    const fileClients = [];
    for (let i = 0; i < 3; i++) {
      const fileClient = fileSystemClient.getFileClient(
        getUniqueName(`file${i}`)
  ...
  it("listDeletedPaths and listPaths with recreating file after deletion @loki @sql", async () => {
    const fileClients = [];
    for (let i = 0; i < 3; i++) {
      const fileClient = fileSystemClient.getFileClient(
        getUniqueName(`file${i}`)
    ...
    assert.deepStrictEqual(
      listPathResult.pathItems!.length,
      fileClients.length
    );
    assert.ok(fileClients[0].url.indexOf(listPathResult.pathItems![0].name!));
  });

  it("listDeletedPaths with recreating and deletion again @loki @sql", async () => {
    const fileClients = [];
    for (let i = 0; i < 3; i++) {
      const fileClient = fileSystemClient.getFileClient(
        getUniqueName(`file${i}`)
  ...
  it("listDeletedPaths with default parameters - empty path shouldn't throw error @loki @sql", async () => {
    const fileClients = [];
    for (let i = 0; i < 3; i++) {
      const fileClient = fileSystemClient.getFileClient(
        getUniqueName(`file${i}`)
    ...
    const result = (
      await fileSystemClient.listDeletedPaths({ prefix: "" }).byPage().next()
    ).value;
  ...
  it("listDeletedPaths with all parameters configured and byPage with continuationToken @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator for listDeletedPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator(generator .next() syntax) for listDeletedPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator(byPage()) for listDeletedPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Verify PagedAsyncIterableIterator(byPage() - continuationToken) for listDeletedPaths @loki @sql", async () => {
    const fileClients = [];
    const prefix = "file";
    const metadata = {
      keya: "a",
  ...
  it("Undelete file and directory @loki @sql", async () => {
    ...
    assert.ok(
      directoryUndeleteResponse.pathClient instanceof DataLakeDirectoryClient
    );
  ...
  it("Undelete file and directory - recreate and delete path and undelete the path with first deletionid @loki @sql", async () => {
    const fileName = getUniqueName(`file`);
    const fileClient = fileSystemClient.getFileClient(fileName);
    await fileClient.create();
    const fileDeleteResponse = await fileClient.delete();
  ...
  it("Undelete file and directory - recreate and delete path and undelete the path twice @loki @sql", async () => {
    const fileName = getUniqueName(`file`);
    const fileClient = fileSystemClient.getFileClient(fileName);
    await fileClient.create();
    ...
    try {
      await fileSystemClient.undeletePath(
        fileName,
        firstDeleteResponse.deletionId ?? ""
      );
  ...
  it("Undelete file and directory with deleteIfExists @loki @sql", async () => {
    const fileName = getUniqueName(`file`);
    const fileClient = fileSystemClient.getFileClient(fileName);
    await fileClient.create();
  ...
  it("Undelete file and directory special char @loki @sql", async () => {
    const fileNames = [
      "!'();[]@&%=+$,#äÄöÖüÜß;",
...
function checkIfShouldSkip(context: Context) {
  if (context.test!.parent!.title.indexOf("soft delete") > -1) {
    context.skip();
  }
}

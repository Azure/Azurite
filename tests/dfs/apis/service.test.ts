...
describe("ServiceAPIs", () => {
  ...
  it(`getUserDelegationKey with Key credential should fail @loki @sql`, async () => {
    ...
    try {
      ...
      assert.fail(
        "Should fail to invoke getUserDelegationKey with account key credentials"
      );
    } catch (error) {
      assert.strictEqual(
        (error as any).details.AuthenticationErrorDetail,
  ...
  it(`getUserDelegationKey with SAS token credential should fail @loki @sql`, async () => {
    ...
    try {
      ...
      assert.fail(
        "Should fail to invoke getUserDelegationKey with SAS token credentials"
      );
    } catch (error) {
      assert.strictEqual(
        (error as any).details.AuthenticationErrorDetail,
  ...
  it("Get Blob service stats negative @loki @sql", async () => {
    await serviceClient.getStatistics().catch((err) => {

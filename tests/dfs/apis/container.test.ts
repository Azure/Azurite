...
describe("ContainerAPIs", () => {
  ...
  it("getProperties should return 404 for non existed container @loki @sql", async () => {
    const nonExistedContainerURL =
      serviceClient.getContainerClient("404container-a");
  ...
  it("create with invalid container name @loki @sql", async () => {
    ...
    let expectedError = StorageErrorFactory.getInvalidResourceName(context);
    ...
    for (const item in invalidName) {
      try {
        ...
        assert.strictEqual(error.code, expectedError.errorCode);
        assert.strictEqual(
    ...
    expectedError = StorageErrorFactory.getOutOfRangeName(context);
    for (const item in invalidName) {
      try {
        let cURL = serviceClient.getContainerClient(invalidName[item]);
        const result = await cURL.create();
  ...
  it("list container should success with include as empty string or deleted @loki @sql", async () => {
    ...
    let pipeline = newPipeline(new AnonymousCredential(), {
      retryOptions: { maxTries: 1 },
      // Make sure socket is closed once the operation is done.
      keepAliveOptions: { enable: false }
    });
    ...
    let serviceClientForOptions = new BlobServiceClient(
      `${baseURL}?${sas}`,
      pipeline
    );
    ...
    serviceClientForOptions = new BlobServiceClient(
      `${baseURL}?${sas}`,
      pipeline
    );
  ...
  it("list container should success with different include string @loki @sql", async () => {
    ...
    let ContainerClientForOptions =
      serviceClientForOptions.getContainerClient(containerName);

    // list blob with empty include
    let result = (
      await ContainerClientForOptions.listBlobsFlat({
        includeMetadata: true
      })
        .byPage()
        .next()
  ...
  it.skip("List blob should success with '+' in query @loki @sql", async () => {
    const blobClients = [];
    let blobNames: Array<string> = ["block blob/abc-001", "block blob/abc-002"];
    ...
    const containerClientForOptions =
      serviceClientForOptions.getContainerClient(containerClient.containerName);

...
- Bumped the transitive `uuid` dependency (via `@azure/ms-rest-js` and `sequelize`) from 8.3.2 to 11.1.1 through an npm `overrides` entry, remediating GHSA-w5hq-g745-h8pq / CVE-2026-41907 (missing buffer bounds check in the `v3()`, `v5()` and `v6()` APIs). Added unit coverage asserting the resolved `uuid` version is patched and that out-of-bounds buffer writes throw `RangeError`.

...
- Bumped `mysql2` from 3.23.2 to 3.23.3. `mysql2` is the Sequelize driver used by the SQL-based blob metadata store (`mysql://` connection URIs supplied via the `AZURITE_DB` environment variable); the patch drops the transitive `denque` dependency and moves to `iconv-lite` `^0.7.3`, which dedupes with the copy already used by `express` and `tedious`. No production code changes were required; added `tests/blob/unit/sqlBlobMetadataStore.unit.test.ts` covering driver resolution, the default charset/collation/pool options, the mssql-only encryption path, and mysql2 value escaping, and verified the `@sql` blob suite against a MySQL container.

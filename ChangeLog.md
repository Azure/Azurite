...
- Added a version-agnostic upgrade/persistence compatibility test suite (`tests/upgrade/`, run via `npm run test:upgrade`, `test:upgrade:docker`, `test:upgrade:vsix`) that installs the latest published Azurite (npm and Docker/MCR image), seeds blob (block/append/page, txt/json/csv/xml/binary), queue, and table data, upgrades in place to the local build, and verifies byte-for-byte / value-for-value integrity. Separately, a VSIX lifecycle test installs/activates/starts/stops the latest published Marketplace VSIX and the locally packaged VSIX (persistence-upgrade validation is npm/Docker-only for now). Added dev dependency `@vscode/test-electron` for the VSIX lifecycle test, and a dedicated `.github/workflows/UpgradeCompatibility.yml` CI workflow that runs on merge to `main` and on demand.

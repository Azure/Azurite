...
- Bumped `mysql2` from 3.23.2 to 3.23.3 (Sequelize driver for the SQL-based blob metadata store's `mysql://` connections) for upstream connection-pool fixes. No production code changes; added `tests/blob/unit/sqlBlobMetadataStore.unit.test.ts` and `tests/blob/sqlBlobMetadataStorePool.test.ts` (`@sql`) to cover driver resolution and pooled connections.

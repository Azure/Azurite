...
- Updated lockfile-resolved `mysql2` from 3.23.4 to 3.24.4 to correct three-byte length-coded parameter encoding, improve SQL metadata-store performance, include an unrelated `PoolCluster` typings fix (not used by Azurite), and pick up upstream patch fixes; added SQL pool regression coverage for large bound parameters.

...
- Added an npm `overrides` entry pinning the transitive `@opentelemetry/core` dependency (via `applicationinsights`) to `^2.8.0`, remediating GHSA-8988-4f7v-96qf / CVE-2026-54285 (unbounded memory allocation when `W3CBaggagePropagator.extract()` parses oversized `baggage` headers).

...
- Applied `npm audit fix`: updated the lockfile-resolved `mocha` dev dependency from `12.0.0-rc.5` to `12.0.0-rc.6` and deduplicated the transitive `serialize-javascript` and `iconv-lite` copies. Relaxed the `serialize-javascript` override from the exact `7.0.3` pin to `^7.0.7` (resolves to 7.1.0), because 5.0.0 - 7.0.4 are affected by the CPU-exhaustion denial of service GHSA-qj8w-gfj5-8c6v. Added package-lock coverage in `tests/packageScripts.test.ts` asserting that every resolved `serialize-javascript` copy is at least 7.0.5 and that overridden packages resolve to a single version.

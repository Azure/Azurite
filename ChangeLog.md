...
- Bumped `@vscode/test-electron` dev dependency from `^2.4.1` to `^3.1.0`, used by the `test:upgrade:vsix` VSIX lifecycle tests. The `downloadAndUnzipVSCode`, `resolveCliArgsFromVSCodeExecutablePath`, and `runTests` APIs consumed in `tests/upgrade/vsixLifecycle/installAndRunVsixSession.ts` are unchanged between these versions; no code changes were required.

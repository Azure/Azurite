...
- Bumped `mysql2` from 3.23.2 to 3.23.3. `mysql2` is the Sequelize driver used by the SQL-based blob metadata store (`mysql://` connection URIs supplied via the `AZURITE_DB` environment variable), which runs a connection pool of up to 20 connections. The patch fixes connection-pool behavior relevant to that usage (each pooled connection now gets its own config copy, query dispatch errors are propagated instead of thrown, and `connectTimeout` stays active until the handshake completes), replaces the transitive `denque` dependency with an internal ring buffer, and moves to `iconv-lite` `^0.7.3`, which dedupes with the copy already used by `express` and `tedious`. No production code changes were required; added `tests/blob/unit/sqlBlobMetadataStore.unit.test.ts` covering mysql2 driver resolution, the default charset/collation/pool options, the mssql-only encryption path, and mysql2 value escaping, plus `tests/blob/sqlBlobMetadataStorePool.test.ts` (`@sql`) exercising concurrent metadata operations across pooled connections and error propagation from a pooled connection.

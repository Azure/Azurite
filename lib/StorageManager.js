...
class StorageManager {
    ...
    putBlock(request) {
        ...
        if (blobProxy === undefined) {
            ...
            this._createOrUpdateBlob(coll, parentBlobRequest.id);
        }
        // Storing block information in DB.
        const blockProxy = this._createOrUpdateBlob(coll, request, request.id);
    ...
    putBlockList(request) {
        ...
        const coll = this.db.getCollection(request.containerName);
        const blobProxy = this._createOrUpdateBlob(coll, request, request.id);
    ...
    snapshotBlob(request) {
        ...
        // TODO: Refactor into env once publicName() and diskStorageUri is gone
        const destPath = path.join(env.localStoragePath, request.containerName, env.snapshotFilename(request.blobName, snapshotProxy.original.snapshotDate));
        return fs.copy(request.fileName, destPath)
            .then(() => {

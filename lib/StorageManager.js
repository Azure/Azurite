    ...
    copyBlob(request) {
        ...
        to.on('finish', () => {
            if (blobProxyDestination.original.copyStatus !== CopyStatus.FAILED) {
                ...
                blobProxyDestination.original.copySource = sourceProxy.original.uri; // "/devstoreaccount1/testc8e59d8b2da1d4db2a93419c663305d94/source"
                const { sourceContainerName, sourceBlobName } = request.copySourceName();
                blobProxyDestination.original.copySource = `http://localhost/devstoreaccount1/${sourceContainerName}/${sourceBlobName}`;

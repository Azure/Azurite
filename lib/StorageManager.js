...
class StorageManager {
    ...
    _createOrUpdateBlob(request) {
        const coll = this.db.getCollection(request.containerName);
        const blob = coll ? coll.chain().find({ 'name': { '$eq': request.blobName } }).data() : null;
        const blobProxy = null;
        if (blob !== null) {
            coll.chain().find({ 'name': { '$eq': request.blobName } }).remove();
        }
        const entity = StorageEntityGenerator.generateStorageEntity(request);
        blobProxy = new BlobProxy(coll.insert(entity));
        coll.update(blobProxy.original);
        return blobProxy;
    }
    ...
    putBlob(request) {
        ...
        const blobProxy = this._createOrUpdateBlob(request);

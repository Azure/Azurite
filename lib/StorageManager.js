...
class StorageManager {
    ...
    createBlockBlob(container, blob) {
        let containerName = path.join(env.localStoragePath, container);
        return fs.statAsync(containerName)
            .then((stat) => {
        ...
        // Check if container exists (fail if it doesn't) DONE
        // Create file (fs.outputFileAsync(filePath, data, {encoding:'utf8'}))
        // Create db entry with attribs (blobProps, httpProps, metaProps)
        // blobProps mapping (x-ms-* always overrides http header values)

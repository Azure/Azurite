...
class StorageManager {
    ...
    snapshotBlob(containerName, blob) {
        return BbPromise.try(() => {
    ...
    _getStoragePath(containerName, blob) {
        if (blob.isVirtualDirectory()) {
            const containerPath = path.join(env.virtualDirPath, containerName);
            const blobPath = path.join(containerPath, Buffer.from(blob.name, 'utf8').toString('base64'));
            return blobPath;
        }
        const containerPath = path.join(env.localStoragePath, containerName);
        const blobPath = path.join(containerPath, blob.name);
        return this._escapeBlobDelimiter(blobPath);
    }

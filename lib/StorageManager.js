...
class StorageManager {
    ...
    init(localStoragePath) {
        ...
        return fs.statAsync(env.azuriteDBPath)
            ...
            .then((data) => {
                ...
                if (!this.db.getCollection(StorageTables.Snapshots)) {
                    this.db.addCollection(StorageTables.Snapshots);
                }
    ...
    snapshotBlob(containerName, requestBlob) {
        return BbPromise.try(() => {
            ...
            const collSnapshot = this.db.getCollection(StorageTables.Snapshots),
                updateBlob = this._getCollectionAndBlob(containerName, blob.name).blob;

            if (requestBlob.metaProps) {
                updateBlob.metaProps = requestBlob.metaProps;
                updateBlob.ETag += 1;
                updateBlob.httpProps['Last-Modified'] = requestBlob.httpProps['Last-Modified'];
            } else {
                requestBlob.httpProps['Last-Modified'] = updateBlob.httpProps['Last-Modified'];
                requestBlob.setSnapshotDate();
            }
            updateBlob.name = `${containerName}-${requestBlob.publicName()}`;
            // Removing internal LokiJS data structures (ID, meta) since this object will be inserted into another collection.
            delete updateBlob['$loki'];
            delete updateBlob.meta;
            collSnapshot.insert(updateBlob);
            
            const destPath = path.join(env.snapshotPath, containerName, updateBlob.name);
            return fs.ensureDir(path.join(env.snapshotPath, containerName))
                .then(() => {
                    return fs.copy(this._getStoragePath(containerName, requestBlob), destPath);
                })
                .then(() => {
                    return {
                        httpProps: updateBlob.httpProps,

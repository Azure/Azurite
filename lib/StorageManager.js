...
class StorageManager {
    ...
    leaseContainer(containerRequest) {
        return BbPromise.try(() => {
            ...
            container.leaseState = this._updateLeaseState(container.leaseState, container.leaseValidUntil, container.leaseBrokenAt)
            ...
            switch (leaseAction) {
                ...
                case 'break':
                    ...
                    if (!leaseBreakPeriod) {
                        container.leaseBrokenAt = (container.leaseValidUntil === -1) ? Date.now() : container.leaseValidUntil;
                    }
    ...
    _updateLeaseState(leaseState, leaseValidUntil, leaseBrokenAt) {
        switch (leaseState) {
            ...
            // Has lease expired?
            case 'leased':
                return (leaseValidUntil <= Date.now()) ? 'expired' : 'leased';
        }
    }

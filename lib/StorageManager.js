...
class StorageManager {
    ...
    putBlob(containerName, blob, body) {
        return BbPromise.try(() => {
            ...
            // Fixme: 
            // On Linux a trailing / results in an exception since some/path/ cannot be created
            // On Windows a trailing / is ignored. some/path/ results in /some/path being created as a blob of size 0
            // => I think that a PUT call to a path with a trailing / should result in a NOOP. Virtual dirs solely exist
            //    on an SDK / Client level and serve only as a prefix of a blob name. As long as no actual blob is created with that
            //    prefix there is no need to create anything on the server side. This is also backed by the way how 
            //    Storage Emulator and our SDKs work: creation of a virtual dir (without any blobs in it) do not send any call
            //    to the backend.

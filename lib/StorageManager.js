...
class StorageManager {
    ...
    getContainerMetadata(containerName, options) {
        return BbPromise.try(() => {
            ...
            res.container.leaseState = this._updateLeaseState(container.leaseState, container.leaseExpiredAt, container.leaseBrokenAt);
            validationContext.run(ContainerLeaseUsageValidation, {
    ...
    setContainerAcl(container, signedIdentifiers, options) {
        return BbPromise.try(() => {
            const validationContext = new RootValidator({
                containerName: container.name,
                collection: this.db.getCollection(StorageTables.Containers),
            ...
            updateContainer.leaseState = this._updateLeaseState(container.leaseState, container.leaseExpiredAt, container.leaseBrokenAt);
            validationContext.run(ContainerLeaseUsageValidation, {
        ...
        return BbPromise.try(() => {
            ...
            switch (leaseAction) {
                case 'acquire':
                    ...
                    break;
                case 'renew':
                    container.leaseExpiredAt = (container.leaseDuration === -1) ? -1 : Date.now() + container.leaseDuration * 1000;
                ...
                case 'break':
                    ...
                    if (!leaseBreakPeriod) {
                        container.leaseBrokenAt = (container.leaseExpiredAt === -1) ? Date.now() : container.leaseExpiredAt;
                    }
                    const span = container.leaseExpiredAt - Date.now();
                    if (span > leaseBreakPeriod) {
                        container.leaseBrokenAt = Date.now() + leaseBreakPeriod;
                    } else {
                        container.leaseBrokenAt = Date.now() + span;
                    }
                    container.leaseState = 'breaking';
                    const leaseTimeRemaining = Math.floor((container.leaseBrokenAt - Date.now()) / 1000);
                    result.props['x-ms-lease-time'] = (leaseTimeRemaining > 0) ? leaseTime : 0;
    ...
    _updateLeaseState(leaseState, leaseExpiredAt, leaseBrokenAt) {
        switch (leaseState) {
            ...
            case 'breaking':
                return (leaseBrokenAt <= Date.now()) ? 'broken' : 'breaking';
            // Has lease expired?
            case 'leased':
                return (leaseExpiredAt <= Date.now()) ? 'expired' : 'leased';
            default:
                return leaseState;
        }
    }

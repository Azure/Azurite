...
class StorageManager {
    ...
    createContainer(request) {
        ...
        return fs.mkdirAsync(request.fullPath())
            .then(() => {
                const httpProps = {};
                httpProps[N.LAST_MODIFIED] = containerProxy.lastModified();
                return new AzuriteResponse({ httpProps: httpProps, statusCode: 201 });
            });
    }
    ...
    deleteContainer(request) {
        ...
        return fs.remove(request.fullPath())
            .then(() => {
                return new AzuriteResponse({ statusCode: 202 });
            });
    }
    ...
    listContainer(prefix, maxresults) {
        return BbPromise.try(() => {
            ...
            let result = tables.chain()
                ...
                .limit(maxresults)
                .data();
            return new AzuriteResponse({ payload: payload, statusCode: 200 });
        });
    }

    // RESUME: Hier gehts weiter!
    putBlob(containerName, blob, body) {
        return BbPromise.try(() => {
            ...
            if (blobResult.length === 0) {
                ...
                updateBlob.ETag = `${updateBlob.name}-${updateBlob.meta.revision}`;
                response.ETag = updateBlob.ETag;
                response['Last-Modified'] = blob.httpProps['Last-Modified'];
            }
    ...
    putAppendBlock(containerName, blob, body) {
        return BbPromise.try(() => {
            const response = {};
            // Fixme: This call is not exception-safe since container may not exist, validation is missing / too late
        ...
        return BbPromise.try(() => {
            ...
            new RootValidator({
                ...
                .run(BlobLeaseUsageValidation, {
                    ...
                    blob: coll.chain().find({ 'name': { '$eq': requestBlob.publicName() } }).data()[0]
                })
                .run(ConditionalRequestHeadersVal, {
                    updateItem: coll.chain().find({ 'name': { '$eq': requestBlob.publicName() } }).data()[0],
                    operationType: 'read'
                });
            ...
            response.httpProps.ETag = blob.ETag;
    ...
    setBlobMetadata(containerName, blob) {
        return BbPromise.try(() => {
            ...
            blobToUpdate.ETag = `${blobToUpdate.name}-${blobToUpdate.meta.revision}`
            return {
                ETag: blobToUpdate.ETag,
    ...
    getBlobMetadata(containerName, requestBlob) {
        return BbPromise.try(() => {
            ...
            clonedHttpProps.ETag = blob.ETag;
    ...
    setContainerMetadata(containerRequest) {
        return BbPromise.try(() => {
            ...
            container.ETag = `${container.name}-${container.meta.revision}`
            return {
    ...
    getContainerMetadata(containerName, options) {
        return BbPromise.try(() => {
            ...
            httpProps.ETag = res.container.ETag;
            return {
                httpProps: httpProps,
                metaProps: metaProps
            };
            ...
            return new BbPromise((resolve, reject) => {
                writeStream
                    ...
                    .on('finish', () => {
                        ...
                        updatedBlob.ETag = `${updatedBlob.name}-${updatedBlob.meta.revision}`;
                        updatedBlob.httpProps.ETag = updatedBlob.ETag;

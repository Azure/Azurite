...
class StorageManager {
    ...
    putPage(request) {
        ...
        this._updatePageRanges(coll, pageRanges, startByte, endByte, request.pageId);
    ...
    snapshotBlob(request) {
        const { coll, blobProxy } = this._getCollectionAndBlob(request.containerName, request.originId);
        ...
        snapshotEntity.originId = request.originId;
        ...
        return fs.copy(request.parentUri, request.uri)
            .then(() => {
    ...
    _updatePageRanges(coll, pageRanges, startByte, endByte, pageId) {
    ...
    _buildBlockListQuery(containerName, blobName, blockListType) {
        let query = {
            '$and': [{
                parentId: `${containerName}-${blobName}`
            },
            {

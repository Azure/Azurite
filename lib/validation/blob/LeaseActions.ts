...
class LeaseActions {
    ...
    validate({ request = undefined, containerProxy = undefined, blobProxy = undefined }) {
        const leaseAction = request.httpProps[N.LEASE_ACTION],
            ...
            proxy = (request instanceof AzuriteBlobRequest) ? blobProxy : containerProxy;
        ...
        switch (proxy.original.leaseState) {
            ...
            case LeaseStatus.EXPIRED:
                ...
                } else if (leaseAction === LeaseAction.RENEW && request instanceof AzuriteBlobRequest && leaseId === proxy.original.leaseId && proxy.original.leaseETag !== proxy.original.etag) {
                    throw new AError(ErrorCodes.LeaseNotPresentWithLeaseOperation)
                }

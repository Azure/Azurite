...
class AppendBlobSanity {
    ...
    validate(options) {
        ...
        if (blobAppendPos && blobAppendPos !== options.updateBlob.size) {
            throw new AError('PreconditionFailed', 412, 'One of the XML nodes specified in the request body is not supported.');
        }
        ...
        if (options.updateBlob.httpProps['x-ms-blob-committed-block-count'] > 50000) {
            throw new AError('BlockCountExceedsLimit', 409, 'The committed block count cannot exceed the maximum limit of 50,000 blocks.');
        }
        return options;
    }
}

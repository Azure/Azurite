...
class Environment {
    ...
    storageUrl(port, container, blob) {
        return (blob.isVirtualDirectory() && !blob.isSnapshot()) 
            ? `http://localhost:${port}/blobs/${this.virtualDirUri}/${container}/${Buffer.from(blob.name, 'utf8').toString('base64')}`
            : (blob.isSnapshot()) 
                ? `http://localhost:${port}/blobs/${this.snapshotUri}/${container}/${Buffer.from(blob.name + blob.snapshotDate , 'utf8').toString('base64')}`

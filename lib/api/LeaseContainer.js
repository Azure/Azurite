...
class LeaseContainer extends StandardHandler {
    ...
    processImpl(request, res) {
        storageManager.leaseContainer(request)
            .then((response) => {
                response.addHttpProperty(N.leaseId, response.proxy.original.leaseId);
                // Refactor me: now should be initialized in request object by dedicated midleware module 
                const now = Date.now();
                const leaseTimeRemaining = Math.floor((response.proxy.original.leaseBrokenAt - now) / 1000);

    ...
    process(request, res) {
        ...
        storageManager.getBlob(request)
            .then((response) => {
                ...
                // Fixme: MD5 calculation of page blobs seem to be wrong since they do not match
                // client-side generated MD5 values.
                // if (response.proxy.original.entityType !== EntityType.PageBlob) {
                ...
                if (range && request.httpProps[N.RANGE_GET_CONTENT_MD5]) {
                    ...
                    req(this._createRequestHeader(env.webStorageUri(env.port, request), range))
                        .on('response', (staticResponse) => {
                            ...
                            if (range) {
                                // RESUME: What is difference between old version on new one? New one fails due to wrong md5 checksum which I cannot find anywhere.
                                // Corrent one is being sent to client.. strange 

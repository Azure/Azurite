...
class QueryEntities {
  ...
  _createResponsePayload(payload, tableName, accept, singleEntity) {
    ...
    this._setResponseODataMetadata(response, tableName, accept, singleEntity);
  ...
  _setResponseODataMetadata(response, tableName, accept, singleEntity) {
    if (accept === ODataMode.NONE) {
      return;
    }
  
    // multiple elements http://127.0.0.1:10002/devstoreaccount1/$metadata#SomeTableName
    // single element http://127.0.0.1:10002/devstoreaccount1/$metadata#SomeTableName/@Element
    const elementMetadataPart = singleEntity ? `/@Element` : ``;
  
    response["odata.metadata"] = `http://127.0.0.1:10002/devstoreaccount1/$metadata#${tableName}${elementMetadataPart}`;
  }
}

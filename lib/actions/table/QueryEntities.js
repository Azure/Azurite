...
class QueryEntities {
    ...
    _createResponsePayload(payload, tableName, accept, singleEntity) {
        ...
        if(singleEntity){
            response['PartitionKey'] = payload.partitionKey;
            response['RowKey'] = payload.rowKey;
            for(var attrib in payload.attribs(accept)){
                response[attrib] = attrib;
            }
        }
        else if(payload.length === 1){
            response['PartitionKey'] = payload[0].partitionKey;
            response['RowKey'] = payload[0].rowKey;
            for(var attrib in payload[0].attribs(accept)){
            ...
            for (const item of payload) {
                var entity = item.attribs(accept);

...
class Blob extends StorageItem {
     ...
     * 
     * @param {"snapshot" || "virtualdirectory"} type - optional: if specified overrules name creation. 
     * @returns 
     * 
     * @memberof Blob
     */
    publicName(type) {
        if (this.isSnapshot() || type === 'snapshot') {
            return `${this.name}-${Date.parse(this.snapshotDate || this.httpProps['Last-Modified'])}`;
        }
        if (this.isVirtualDirectory() || type === 'virtualdirectory') {

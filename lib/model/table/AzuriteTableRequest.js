...
class AzuriteTableRequest {
  ...
  _parseAccept(value) {
    ...
    if (value.includes(`odata=nometadata`)) return ODataMode.NONE;
    if (value.includes(`odata=minimalmetadata`)) return ODataMode.MINIMAL;
    if (value.includes(`odata=fullmetadata`)) return ODataMode.FULL;
  }

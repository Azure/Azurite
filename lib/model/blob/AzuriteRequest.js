...
class AzuriteRequest {
  constructor({
    ...
    if (req === undefined) {
      throw new InternalAzuriteError(
        "AzuriteRequest: req cannot be undefined!"
      );
    }
  ...
  _initHttpProps(httpHeaders) {
    ...
    this.httpProps[N.ACCESS_CONTROL_REQUEST_HEADERS] =
      httpHeaders["access-control-request-headers"];
    // x-ms-* attributes have precedence over according HTTP-Headers
    this.httpProps[N.CONTENT_TYPE] =
      httpHeaders["x-ms-blob-content-type"] ||
      httpHeaders["content-type"] ||
      "application/octet-stream";
    this.httpProps[N.CONTENT_ENCODING] =
      httpHeaders["x-ms-blob-content-encoding"] ||
      httpHeaders["content-encoding"];
    this.httpProps[N.CONTENT_DISPOSITION] =
      httpHeaders["x-ms-blob-content-disposition"] ||
      httpHeaders["content-disposition"];
    this.httpProps[N.CACHE_CONTROL] =
      httpHeaders["x-ms-blob-cache-control"] || httpHeaders["cache-control"];
    this.httpProps[N.CONTENT_LANGUAGE] =
      httpHeaders["x-ms-blob-content-language"] ||
    ...
    this.httpProps[N.RANGE_GET_CONTENT_MD5] =
      httpHeaders["x-ms-range-get-content-md5"];
    ...
    this.httpProps[N.LEASE_ACTION] = httpHeaders["x-ms-lease-action"]
      ? httpHeaders["x-ms-lease-action"].toLowerCase()
      : undefined;
    this.httpProps[N.LEASE_DURATION] = httpHeaders["x-ms-lease-duration"];
    this.httpProps[N.LEASE_BREAK_PERIOD] =
      httpHeaders["x-ms-lease-break-period"];
    ...
    this.httpProps[N.SOURCE_IF_MODFIFIED_SINCE] =
      httpHeaders["x-ms-source-if-modified-since"];
    this.httpProps[N.SOURCE_IF_UNMODIFIED_SINCE] =
      httpHeaders["x-ms-source-if-unmodified-since"];
    this.httpProps[N.SOURCE_IF_MATCH] = httpHeaders["x-ms-source-if-match"];
    this.httpProps[N.SOURCE_IF_NONE_MATCH] =
      httpHeaders["x-ms-source-if-none-match"];
    ...
    this.httpProps[N.BLOB_PUBLIC_ACCESS] =
    ...
    this.httpProps[N.IF_SEQUENCE_NUMBER_LE] =
      httpHeaders["x-ms-if-sequence-number-le"];
    this.httpProps[N.IF_SEQUENCE_NUMBER_LT] =
      httpHeaders["x-ms-if-sequence-number-lt"];
    this.httpProps[N.IF_SEQUENCE_NUMBER_EQ] =
      httpHeaders["x-ms-if-sequence-number-eq"];
    this.httpProps[N.BLOB_ACCESS_TIER] =
      httpHeaders['x-ms-access-tier'];
    this.httpProps[N.BLOB_REHYDRATE] =
      httpHeaders['x-ms-archive-status'];

    Object.keys(this.httpProps).forEach((key) => {
      if (
        this.httpProps[key] === undefined ||
        this.httpProps[key] === "undefined"
      ) {

...
class StorageEntityGenerator {
  ...
  generateStorageEntity(request) {
    ...
    if (request.entityType === EntityType.Container) {
      ...
      entity.accessTier = 'Hot'; //request.httpProps[N.BLOB_ACCESS_TIER]; entity.accessTier === undefined ? delete entity.accessTier : (() => {/*NOOP*/ });
      entity.rehydrate = request.httpProps[N.BLOB_REHYDRATE];
      entity.rehydrate === undefined
        ? delete entity.rehydrate
        : () => {
            /*NOOP*/
          };
      entity.timeToRehydrate = 0;
    }
  ...
  clone(o) {
    ...
    if (o.entityType !== EntityType.Container) {
      ...
      if (o.accessTier) {
        copy.accessTier = o.accessTier;
      }
      if (o.rehydrate) {
        copy.rehydrate = o.rehydrate;
      }
      if (o.timeToRehydrate) {
        copy.timeToRehydrate = o.timeToRehydrate;
      }
    }
    return copy;
  }

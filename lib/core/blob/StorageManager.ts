...
class StorageManager {
  ...
  createContainer(request) {
    ...
    return BbPromise.resolve(
      new AzuriteResponse({ proxy: containerProxy, cors: request.cors })
    );
  }
    ...
    copyBlob(request) {
        ...
        to.on('finish', () => {
            if (blobProxyDestination.original.copyStatus !== CopyStatus.FAILED) {
                ...
                blobProxyDestination.original.contentType = sourceProxy.original.contentType;
                blobProxyDestination.original.contentEncoding = sourceProxy.original.contentEncoding;
                blobProxyDestination.original.contentLanguage = sourceProxy.original.contentLanguage;
                blobProxyDestination.original.cacheControl = sourceProxy.original.cacheControl;
                blobProxyDestination.original.contentDisposition = sourceProxy.original.contentDisposition;

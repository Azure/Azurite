...
class TableStorageManager {
    ...
    queryEntities(request) {
        ...
        let coll = this.db.getCollection(request.tableName);
        let findExpr = {};
        ...
        let find;
        if (request.filter) {
            find = coll.chain().find(findExpr).where((item) => {
                return eval(request.filter);
            }).data();
        }
        else{
             find = coll.chain().find(findExpr).data();
        }
        // ToDo: Check correct Chain usage here!
        let result = find;
        if(find.length == 0){

...
validations[Operations.Account.LIST_CONTAINERS] = () => {
  // NO VALIDATIONS
};
...
validations[Operations.Blob.PUT_BLOB] = (request, valContext) => {
  valContext
    .run(ServiceSignatureValidation, { sasOperation: ServiceSAS.Blob.WRITE })
    .run(MD5Val)
    .run(ContainerExistsVal)
    .run(BlobNameVal)
...
validations[Operations.Blob.APPEND_BLOCK] = (request, valContext) => {
  valContext
    .run(ServiceSignatureValidation, { sasOperation: ServiceSAS.Blob.ADD })
    .run(BlobExistsVal)
...
validations[Operations.Blob.DELETE_BLOB] = (request, valContext) => {
  valContext
    .run(ServiceSignatureValidation, { sasOperation: ServiceSAS.Blob.DELETE })
    .run(BlobExistsVal)
...
validations[Operations.Blob.GET_BLOB] = (request, valContext) => {
  valContext
    .run(ServiceSignatureValidation, { sasOperation: ServiceSAS.Blob.READ })
    .run(BlobExistsVal)
...
validations[Operations.Container.LIST_BLOBS] = (request, valContext) => {
  valContext
    .run(ServiceSignatureValidation, { sasOperation: ServiceSAS.Blob.LIST })
    .run(ContainerExistsVal);
};
...
validations[Operations.Blob.SNAPSHOT_BLOB] = (request, valContext) => {
  valContext
    .run(ServiceSignatureValidation, { sasOperation: ServiceSAS.Blob.CREATE })
    .run(ContainerExistsVal)
    .run(BlobExistsVal)
    .run(ConditionalRequestHeadersVal, { usage: Usage.Write })
    .run(BlobLeaseUsageValidation, { usage: Usage.Read });

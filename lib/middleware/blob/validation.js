...
const BbPromise = require("bluebird"),
    ...
    SetTierHeaderValidation = require('./../../validation/blob/SetTierHeaders');
...
validations[Operations.Blob.PUT_BLOB] = (request, valContext) => {
    valContext
        .run(ServiceSignatureValidation, {
            sasOperation: SasOperation.Blob.WRITE
        })
        .run(MD5Val)
        .run(ContainerExistsVal)
        .run(BlobNameVal)
...
validations[Operations.Blob.SET_BLOB_TIER] = (request, valContext) => {
    valContext
        .run(SetTierHeaderValidation)
};

validations[Operations.Blob.APPEND_BLOCK] = (request, valContext) => {
    valContext
        .run(ServiceSignatureValidation, {
            sasOperation: SasOperation.Blob.ADD
        })
        .run(BlobExistsVal)
...
validations[Operations.Blob.DELETE_BLOB] = (request, valContext) => {
    valContext
        .run(ServiceSignatureValidation, {
            sasOperation: SasOperation.Blob.DELETE
        })
        .run(BlobExistsVal)
...
validations[Operations.Blob.GET_BLOB] = (request, valContext) => {
    valContext
        .run(ServiceSignatureValidation, {
            sasOperation: SasOperation.Blob.READ
        })
        .run(BlobExistsVal)
...
validations[Operations.Container.LIST_BLOBS] = (request, valContext) => {
    valContext
        .run(ServiceSignatureValidation, {
            sasOperation: SasOperation.Blob.LIST
        })
        .run(ContainerExistsVal);
};
...
validations[Operations.Blob.SNAPSHOT_BLOB] = (request, valContext) => {
    valContext
        .run(ServiceSignatureValidation, {
            sasOperation: SasOperation.Blob.CREATE
        })
        .run(ContainerExistsVal)
        .run(BlobExistsVal)
        .run(ConditionalRequestHeadersVal, {
...
validations[Operations.Blob.COPY_BLOB] = (request, valContext) => {
    ...
    valContext
        ...
        .run(ContainerExistsVal, {
            containerProxy: sourceContainerProxy
        })
        .run(BlobExistsVal, {
            blobProxy: sourceBlobProxy
        });
    ...
    valContext
        ...
        .run(CompatibleBlobTypeVal, {
            request: {
                entityType: sourceBlobProxy.original.entityType
            },
        })
        .run(ConditionalRequestHeadersVal, {
            usage: Usage.Write

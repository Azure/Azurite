  ...
  BbPromise.try(() => {
    ...
    const accessPolicy = {
      ...
      canonicalizedResource:
        request.query.sr === "c"
          ? `/blob/${storageAccount}/${request.containerName}`
          : `/blob/${storageAccount}/${request.containerName}/${
              request.blobName
            }`,
      id: request.query.si,
    ...
    if (env.accountAuth && req.method !== "HEAD" && request.auth.sasValid === false) {
      ...
      const match = req.headers.authorization.match(`SharedKey ${storageAccount}:(.*)`)
      if (match === null) {
        throw new AError(ErrorCodes.AuthenticationFailed);
      }
...
function _generateAccountSignature(req) {
  ...
  str += `/${storageAccount}${req._parsedUrl["pathname"]}\n`;

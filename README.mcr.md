...
`--skipApiVersionCheck` skip the request API version check. This can also be enabled without changing the command line, by setting the environment variable `AZURITE_SKIP_API_VERSION_CHECK` to `true`, for example `docker run -e AZURITE_SKIP_API_VERSION_CHECK=true ...`. Only the exact value `true` enables it.

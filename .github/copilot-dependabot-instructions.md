...
5. **Changelog** — Add a bullet to ChangeLog.md under ## Upcoming Release → General:, following the style and tone of the existing entries. Include the package name in backticks, the old → new version, the reason for the change, and a concise one-line summary of any code changes made. If the package already has an entry in the Upcoming Release General: section, update the existing bullet instead. Do not add any information stating “No Azurite code changes were needed.”

...
export default class LokiTableMetadataStore implements ITableMetadataStore {
  ...
  private getTableCollectionName(account: string, table: string, batchID?: string): string {
    if (batchID) {
  ...
  public async beginBatchTransaction(account: string, table:string, partitionKey: string, batchID: string, context: Context): Promise<void> {
    const tablesCollection = this.db.getCollection(this.TABLES_COLLECTION);
    const tableDocument = tablesCollection.findOne({
      account,
      table
    ...
    const copiedCollection = this.db.getCollection(
      copyCollectionName
    );
    if (!copiedCollection) {
      throw StorageErrorFactory.getTableNotExist(context);
  ...
  public async endBatchTransaction(account: string, table:string, batchID: string, context: Context): Promise<void> {
    const tableCollectionName = this.getTableCollectionName(account, table, batchID);
    const tableEntityCollection = this.db.getCollection(tableCollectionName);
    if (tableEntityCollection) {
      this.db.removeCollection(tableCollectionName);
    }

...
export class BlobBatchHandler {
  ...
  constructor(
    ...
    const subRequestContextMiddleware = (
      ...
      internnalBlobStorageContextMiddleware(
        new DataLakeContext(locals, DEFAULT_CONTEXT_PATH),
        req,
        res,
        urlbuilder.getHost()!,
        urlbuilder.getPath()!,
    ...
    const subRequestAuthenticationMiddleware = (
      ...
      const context = new DataLakeContext(locals, DEFAULT_CONTEXT_PATH);
  ...
  private async parseSubRequests(
    ...
    for (const subRequest of subRequests) {
      ...
      if (
        operation !== Operation.Path_Delete &&
      ...
      if (previousOperation === undefined) {
        ...
        context.contextId = commonRequestId;
  ...
  public async submitBatch(
    ...
    if (error) {
      ...
      for (const subRequest of subRequests!) {
        ...
        this.logger.info(
          `BlobBatchHandler: completed on subrequest ${
            subRequest.content_id
          } ${subResponse.getHeader("x-ms-request-id")}`,
          context.contextId
        );
      }
    }

    return this.serializeSubResponse(
      perRequestPrefix,
      batchRequestEnding,
      subResponses
    );

...
import { BlobModel } from "../persistence/IDataLakeMetadataStore";
import { toPermissions } from "../storage-file-datalake/transforms";
import {
  BLOB_API_VERSION,
  HeaderConstants,
  ...
  implements IAppendBlobHandler
{
  ...
  public async appendBlock(
    ...
    if (contentLength > MAX_APPEND_BLOB_BLOCK_SIZE) {
      throw StorageErrorFactory.getRequestEntityTooLarge(blobCtx);
    }

    if (contentLength === 0) {
      throw StorageErrorFactory.getInvalidHeaderValue(blobCtx, {
    ...
    if (blob.properties.blobType !== Models.BlobType.AppendBlob) {
      throw StorageErrorFactory.getBlobInvalidBlobType(blobCtx);
    }

    const committedBlockCount = (blob.committedBlocksInOrder || []).length;
    if (committedBlockCount >= MAX_APPEND_BLOB_BLOCK_COUNT) {
      throw StorageErrorFactory.getBlockCountExceedsLimit(blobCtx);
    }

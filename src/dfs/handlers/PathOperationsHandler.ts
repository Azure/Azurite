...
import IDataLakeMetaDataStore, {
  BlobModel
} from "../persistence/IDataLakeMetadataStore";
import {
  ...
  implements IPathOperationsHandler
{
  ...
  async create(
    ...
    if (options.properties) {
      ...
      metaDataValues.forEach((pair) => {
        ...
        const value = Buffer.from(pair.substring(idx + 1), "base64").toString();
   ...
   *
   * @private
   * @param {Models.PathReadOptionalParams} options
  ...
  private async downloadBlockBlobOrAppendBlob(
    ...
    if (rangeStart > blob.properties.contentLength!) {
      throw StorageErrorFactory.getInvalidPageRange(context);
    }
    ...
    const response: Models.PathReadResponse = {
      ...
      ...blob.properties,
      cacheControl:
        context.request!.getQuery("rscc") ?? blob.properties.cacheControl,
      contentDisposition:
        context.request!.getQuery("rscd") ?? blob.properties.contentDisposition,
      contentEncoding:
        context.request!.getQuery("rsce") ?? blob.properties.contentEncoding,
      contentLanguage:
        context.request!.getQuery("rscl") ?? blob.properties.contentLanguage,
      contentType:
        context.request!.getQuery("rsct") ?? blob.properties.contentType,
  ...
  async setProperties(
    ...
    model.properties.contentEncoding = options.pathHTTPHeaders?.contentEncoding;
    model.properties.contentLanguage = options.pathHTTPHeaders?.contentLanguage;
  ...
  async getProperties(
    ...
    const response: Models.PathGetPropertiesResponse = againstMetadata
      ? {
          statusCode: 200,
          eTag: model.properties.etag,
          requestId: context.contextId,
      ...
      : {
          statusCode: 200,
          isIncrementalCopy: model.properties.incrementalCopy,
          eTag: model.properties.etag,
          requestId: context.contextId,
          version: DATA_LAKE_API_VERSION,
          date: context.startTime,
          ...
          ...model.properties,
          cacheControl:
            context.request!.getQuery("rscc") ?? model.properties.cacheControl,
          contentDisposition:
            context.request!.getQuery("rscd") ??
            model.properties.contentDisposition,
          contentEncoding:
            context.request!.getQuery("rsce") ??
            model.properties.contentEncoding,
          contentLanguage:
            context.request!.getQuery("rscl") ??
            model.properties.contentLanguage,
          contentType:
            context.request!.getQuery("rsct") ?? model.properties.contentType
        };
    ...
    if (!model.isDirectory) {
      ...
      response.blobCommittedBlockCount =
        model.properties.blobType === Models.BlobType.AppendBlob
          ? ((model as BlobModel).committedBlocksInOrder || []).length
          : undefined;
    } else {
    ...
    if (options.action === Models.PathGetPropertiesAction.GetAccessControl) {
      response.owner = model.owner;
      response.group = model.group;
      response.permissions = toPermissionsString(model.permissions);
      if (response.permissions || !model.acl) {
        response.aCL = permissionsStringToAclString(response.permissions);
      } else {
        response.aCL = toAclString(model.acl);
      }
    }
  ...
  async delete(
    ...
    const statusCode =
      accept !== undefined && accept.toLowerCase().includes("xml") ? 202 : 200;
  ...
  async flushData(
    ...
    model.properties = {
      ...model.properties,
      cacheControl:
        options.pathHTTPHeaders?.cacheControl || model.properties.cacheControl,
      contentDisposition:
        options.pathHTTPHeaders?.contentDisposition ||
        model.properties.contentDisposition,
      contentEncoding:
    ...
    const key = `${model.accountName}/${model.containerName}/${model.name}/block`;
  ...
  async appendData(
    ...
    // Persist content
    const extent = await this.extentStore.appendExtent(body, context.contextId);
    ...
    // MD5
    const contentMD5 = context.request!.getHeader(HeaderConstants.CONTENT_MD5);
    ...
    const key = `${blob.accountName}/${blob.containerName}/${blob.name}/block`;
    const blockName = getUniqueName(key);
  ...
  async breakLease(options: Models.PathLeaseOptionalParams, context: Context) {
    options.leaseAccessConditions! ||= {};
  ...
  async changeLease(options: Models.PathLeaseOptionalParams, context: Context) {
    if (options.proposedLeaseId === undefined) {
      throw StorageErrorFactory.getLeaseNotPresentWithLeaseOperation(context);
    }
    options.leaseAccessConditions! ||= {};

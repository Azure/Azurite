import StorageError from '../../../blob/errors/StorageError';
...
import DataLakeErrorFactory from '../../errors/StorageErrorFactory';
...
export default function errorMiddleware(
  ...
  err = convertToDataLakeError(err, context);
...
function convertToDataLakeError(e: Error, context: Context): Error {
  if (!("storageErrorCode" in e)) return e;
  const err: StorageError = e as StorageError;

  const [dataLakeErrorCode, dataLakeErrorMsg] =
    DataLakeErrorFactory.blobErrorToDfsError(
      err.storageErrorCode,
      err.storageErrorMessage
    );
  return new DataLakeError(
    ...
    err.storageErrorMessage,
    context,
    err.storageAdditionalErrorMessages
  );
}

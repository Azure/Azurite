...
import { operationDfsToBlob } from '../../utils/operationsMapper';
import { NextFunction } from 'express';
import IRequest from '../../../blob/generated/IRequest';
import UnsupportedRequestError from '../../../blob/generated/errors/UnsupportedRequestError';
import { isURITemplateMatch } from '../../../blob/generated/utils/utils';
...
export default function dispatchMiddleware(
  ...
  for (const key in Operation) {
    if (Operation.hasOwnProperty(key)) {
      ...
      if (res[0] && res[1] > conditionsMet) {
        context.context.dfsOperation = operation;
        context.operation = operationDfsToBlob(operation);
        conditionsMet = res[1];
      }
    }
  }
  ...
  logger.info(
    `DispatchMiddleware: Operation=${Operation[context.context.dfsOperation]}`,
    context.contextId
  );

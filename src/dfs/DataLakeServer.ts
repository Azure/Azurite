...
const BEFORE_CLOSE_MESSAGE = `Azurite DataLake service is closing...`;
const BEFORE_CLOSE_MESSAGE_GC_ERROR = `Azurite DataLake service is closing... Critical error happens during GC.`;
const AFTER_CLOSE_MESSAGE = `Azurite DataLake service successfully closed`;
 ...
 * @class Server
 */
export default class DataLakeServer extends ServerBase implements ICleaner {
  private readonly metadataStore: IDataLakeMetaDataStore;
   ...
   *
   * @param {DataLakeConfiguration} configuration
   * @memberof Server
   */
  constructor(configuration?: DataLakeConfiguration) {
    ...
    const metadataStore: IDataLakeMetaDataStore = new LokiDataLakeMetadataStore(
      configuration.metadataDBPath
      // logger
    );

    const extentMetadataStore: IExtentMetadataStore =
      new LokiExtentMetadataStore(configuration.extentDBPath);
    ...
    const requestListenerFactory: IRequestListenerFactory =
      new DataLakeRequestListenerFactory(
        metadataStore,
        extentStore,
        accountDataStore,
    ...
    const gcManager = new BlobGCManager(
      ...
      (err) => {
        ...
        console.log(BEFORE_CLOSE_MESSAGE_GC_ERROR + ": " + err);
        logger.info(BEFORE_CLOSE_MESSAGE_GC_ERROR + ": " + err);
        this.close().then(() => {
          // tslint:disable-next-line:no-console
  ...
  public async clean(): Promise<void> {
    ...
    throw Error(
      `Cannot clean up DataLake server in status ${this.getStatus()}.`
    );
  }

  protected async beforeStart(): Promise<void> {
    const msg = `Azurite DataLake service is starting on ${this.host}:${this.port}`;
    logger.info(msg);
  ...
  protected async afterStart(): Promise<void> {
    const msg = `Azurite DataLake service successfully listens on ${this.getHttpServerAddress()}`;
    logger.info(msg);
  }

...
import { HeaderConstants, VALID_DATALAKE_AUDIENCES } from "../utils/constants";
...
export default class BlobTokenAuthenticator implements IAuthenticator {
  ...
  public async validate(
    ...
    if (accountProperties === undefined) {
      ...
      throw StorageErrorFactory.ResourceNotFound(blobContext);
    }
    ...
    if (
      ...
      //DataLake
      blobContext.operation === Operation.FileSystem_GetProperties ||
      blobContext.operation === Operation.FileSystem_SetProperties
    ) {
      this.logger.info(
  ...
  public async authenticateBasic(
    ...
    const blobContext = context as DataLakeContext;
    let audMatch = false;
    let m;
    for (const regex of VALID_DATALAKE_AUDIENCES) {
      m = regex.exec(aud);
      if (m !== null) {
        if (m[0] === aud) {

import { BlobPrefixModel } from "./IDataLakeMetadataStore";
...
export default class PageWithDelimiter<BlobType> {
  ...
  private updateFull() {
    this.isFull =
      this.blobItems.length + this.blobPrefixes.size === this.maxResults;
  }
  ...
  private add(name: string, item: BlobType): boolean {
    ...
    if (name < this.latestMarker) {
      throw new Error(
        "add received unsorted item. add must be called on sorted data"
      );
    }

...
import { DEFAULT_FILE_PERMISSIONS } from "../utils/constants";
  ...
  implements IDataLakeMetadataStore, IGCExtentProvider
{
  ...
  public async getContainerACL(
    ...
    const doc =
      forceExist === false
        ? await this.getContainerWithLeaseUpdated(
            account,
            container,
            context,
            false
  ...
  public async list(
    ...
    blobModels = blobModels.filter((model) =>
      this.validateExpireConditions(context, model, false)
    );
  ...
  public async listAllBlobs(
    ...
    for (const doc of docs) {
      ...
      if (!this.validateExpireConditions(undefined, blobDoc, false)) continue;
      blobDoc.properties.contentMD5 = this.restoreUint8Array(
        blobDoc.properties.contentMD5
      );

      filteredDocs.push(blobDoc);
    }

    if (filteredDocs.length <= maxResults) {
      return [filteredDocs, undefined];
    } else {
      const nextMarker = filteredDocs[filteredDocs.length - 2].name;
      filteredDocs.pop();
      return [filteredDocs, nextMarker];
    }
  }
  ...
  public async createBlob(
    ...
    const coll = this.db.getCollection(this.BLOBS_COLLECTION);
    const blobDoc: BlobModel = coll.findOne({
    ...
    const valid = this.validateExpireConditions(context, blobDoc, false);
  ...
  public async createSnapshot(
    ...
    this.validateExpireConditions(context, doc, true);
  ...
  > {
    ...
    const valid = this.validateExpireConditions(undefined, doc, true);
    if (!doc || !valid) {
      return undefined;
    }
  ...
  public async startCopyFromURL(
    ...
    this.validateExpireConditions(context, sourceBlob, true);
  ...
  public async stageBlock(
    ...
    if (!blobDoc || !valid) {
      ...
      if (
        blobDoc.properties.blobType !== Models.BlobType.BlockBlob &&
        (!append || blobDoc.properties.blobType !== Models.BlobType.AppendBlob)
      ) {
  ...
  public async listUncommittedBlockPersistencyChunks(
    ...
    blockDocs = blockDocs.filter((model) =>
      this.validateExpireConditions(undefined, model, false)
    );
  ...
  public async deleteDirectory(
    ...
    await this.deleteBlob(context, account, container, dir.name, options);
  }
  ...
  public async rename(
    ...
    validateWriteConditions(context, options.modifiedAccessConditions, target);
  ...
  private validateExpireConditions(
